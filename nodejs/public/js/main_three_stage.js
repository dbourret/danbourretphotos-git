...
function bindPageNavigation() {
  ...
  document.querySelectorAll("[data-page-target]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const message = btn.dataset.contactMessage || "";
...
function initRestorationShowcase() {
  const viewer = document.getElementById("restoration-stage-viewer");
  const stageImage = document.getElementById("restoration-stage-image");
  const stageLabel = document.getElementById("restoration-stage-label");
  const stageButtons = Array.from(
    document.querySelectorAll("[data-restoration-stage]"),
  );
  const title = document.getElementById("restoration-example-title");
  const description = document.getElementById("restoration-example-description");
  const detailList = document.getElementById("restoration-detail-list");
  const count = document.getElementById("restoration-example-count");
  const prev = document.getElementById("restoration-prev");
  const next = document.getElementById("restoration-next");

  if (!viewer || !stageImage || !stageButtons.length) return;

  const examples = [
    {
      title: "Wedding Reception",
      description:
        "Severe fading repaired, tonal balance restored, and natural color carefully rebuilt.",
      details: [
        "Fading corrected",
        "Contrast restored",
        "Skin tones rebuilt",
        "Background detail recovered",
      ],
      stages: {
        original: {
          src: "images/restoration-samples/wedding-reception-before.jpg",
          alt: "Original faded wedding reception photograph",
          label: "Original",
        },
        bw: {
          src: "images/restoration-samples/wedding-reception-bw.jpg",
          fallback: "images/restoration-samples/wedding-reception-after.png",
          alt: "Restored black-and-white wedding reception photograph",
          label: "Restored B&W",
        },
        color: {
          src: "images/restoration-samples/wedding-reception-after.png",
          alt: "Colorized wedding reception photograph",
          label: "Colorized",
        },
      },
    },
    {
      title: "Wedding Toast",
      description:
        "A heavily faded wedding photograph restored in monochrome and then carefully colorized.",
      details: [
        "Color cast removed",
        "Faces clarified",
      ...
      stages: {
        original: {
          src: "images/restoration-samples/wedding-toast-before.jpg",
          alt: "Original faded wedding toast photograph",
          label: "Original",
        },
        bw: {
          src: "images/restoration-samples/wedding-toast-bw.jpg",
          fallback: "images/restoration-samples/wedding-toast-after.png",
          alt: "Restored black-and-white wedding toast photograph",
          label: "Restored B&W",
        },
        color: {
          src: "images/restoration-samples/wedding-toast-after.png",
          alt: "Colorized wedding toast photograph",
          label: "Colorized",
        },
      },
    },
    {
      title: "Wedding Hallway",
      description:
        "Exposure, facial detail, and clothing texture restored before the final color treatment.",
      details: [
        "Exposure corrected",
        "Facial detail restored",
        "Clothing texture recovered",
        "Natural tones recreated",
      ],
      stages: {
        original: {
          src: "images/restoration-samples/wedding-hallway-before.jpg",
          alt: "Original faded wedding hallway photograph",
          label: "Original",
        },
        bw: {
          src: "images/restoration-samples/wedding-hallway-bw.jpg",
          fallback: "images/restoration-samples/wedding-hallway-after.png",
          alt: "Restored black-and-white wedding hallway photograph",
          label: "Restored B&W",
        },
        color: {
          src: "images/restoration-samples/wedding-hallway-after.png",
          alt: "Colorized wedding hallway photograph",
          label: "Colorized",
        },
      },
    },
    {
      ...
      description:
        "A vintage portrait cleaned, refined in black-and-white, and finished with realistic color.",
      details: [
        "Scratches reduced",
        "Tonal range improved",
        "Portrait detail refined",
        "Realistic color applied",
      ],
      stages: {
        original: {
          src: "images/restoration-samples/family-portrait-before.jpg",
          alt: "Original family portrait",
          label: "Original",
        },
        bw: {
          src: "images/restoration-samples/family-portrait-bw.jpg",
          fallback: "images/restoration-samples/family-portrait-before.jpg",
          alt: "Restored black-and-white family portrait",
          label: "Restored B&W",
        },
        color: {
          src: "images/restoration-samples/family-portrait-after.png",
          alt: "Colorized family portrait",
          label: "Colorized",
        },
      },
    },
  ...
  function setActiveStageButton() {
    stageButtons.forEach((button) => {
      const isActive = button.dataset.restorationStage === currentStage;
  ...
  function loadStageImage(stage) {
    const example = examples[currentProject];
    const stageData = example.stages[stage];
    if (!stageData) return;
    ...
    window.setTimeout(() => {
      ...
      stageImage.onerror = () => {
        if (!triedFallback && stageData.fallback) {
          triedFallback = true;
          stageImage.src = stageData.fallback;
          return;
        }
      ...
      stageImage.src = stageData.src;
      stageImage.alt = stageData.alt;
      stageLabel.textContent = stageData.label;
      currentStage = stage;
  ...
  function renderProject() {
    ...
    count.textContent = `${currentProject + 1} of ${examples.length}`;

    if (detailList) {
      detailList.innerHTML = example.details
        .map((item) => `<li>${item}</li>`)
        .join("");
    }
  ...
  stageButtons.forEach((button) => {
    button.addEventListener("click", () => {
      loadStageImage(button.dataset.restorationStage);
    });
  });

  prev?.addEventListener("click", () => {
    currentProject =
      (currentProject - 1 + examples.length) % examples.length;
    renderProject();
  });

  next?.addEventListener("click", () => {
    currentProject = (currentProject + 1) % examples.length;
    renderProject();
  });

  viewer.addEventListener("click", () => {
    const example = examples[currentProject];
    const stageData = example.stages[currentStage];

    galleryImages = [
      {
        src: stageImage.currentSrc || stageImage.src,
        alt: `${example.title} — ${stageData.label}`,
        dataset: {
  ...
  document.addEventListener("keydown", (event) => {
    const restorationPage = document.getElementById("page-photo-restoration");
    if (!restorationPage?.classList.contains("active") || lightboxIsOpen) return;
...
function initRestorationQuoteForm() {
  const form = document.getElementById("restoration-quote-form");
  if (!form) return;
  ...
  const rightsInput = document.getElementById("quote-rights-confirmation");
  const statusEl = document.getElementById("restoration-quote-status");
  ...
  const allowedMimeTypes = ["image/tiff", "image/png", "image/jpeg"];

  function showQuoteStatus(message, isError = false) {
    if (!statusEl) return;
  ...
  function validateRestorationQuote() {
    clearQuoteStatus();
    clearInvalidFields();
    ...
    if (!file) {
      errors.push("Upload the photograph you would like restored.");
      ...
      const mimeAllowed = !file.type || allowedMimeTypes.includes(file.type);

      if (!extensionAllowed || !mimeAllowed) {
        errors.push("Please upload a TIFF, PNG, or high-quality JPEG image.");
        markInvalid(imageInput);
      }
      ...
      if (file.size > MAX_FILE_SIZE) {
        ...
        errors.push(
          `The selected image is ${sizeInMb} MB. The maximum allowed size is 50 MB.`,
        );
    ...
    if (!description) {
      errors.push("Describe the damage and the changes you would like.");
      markInvalid(descriptionInput);
    } else if (description.length < 20) {
      errors.push(
        "Please provide a little more detail about the damage and requested restoration.",
      );
      markInvalid(descriptionInput);
    }
    ...
    if (!rightsInput?.checked) {
      errors.push(
        "Confirm that you own the photograph or have permission to submit it.",
      );
      markInvalid(rightsInput);
    }
  ...
  imageInput?.addEventListener("change", () => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      showQuoteStatus(
        `This file is ${sizeInMb} MB. Please select an image no larger than 50 MB.`,
        true,
      );
    ...
    if (!allowedExtensions.includes(extension)) {
      showQuoteStatus(
        "Please upload a TIFF, PNG, or high-quality JPEG image.",
        true,
      );
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showQuoteStatus(
        "Uploading your photograph and submitting your request...",
        false,
      );

      const response = await fetch("/api/restoration-quote", {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "The quote request could not be submitted.",
        );
      }

      if (result.requiresOwnershipReview) {
        showQuoteStatus(result.message, true);
        return;
      }

      showQuoteStatus(
        "Your restoration quote request was submitted successfully. You will receive a confirmation by email.",
        false,
      );

      form.reset();
    } catch (error) {
      console.error("Restoration quote submission error:", error);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  initRestorationQuoteForm();
  initRestorationShowcase();
